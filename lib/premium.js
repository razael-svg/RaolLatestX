...
const premium = JSON.parse(fs.readFileSync('./lib/database/userPremium.json'))
/**
 * Add premium user.
 * @param {String} userId 
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./lib/database/userPremium.json", JSON.stringify(_dir));
};
...
const expiredCheck = (haruka, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            fs.writeFileSync('./lib/database/userPremium.json', JSON.stringify(_dir, null, 2))
        }
    }, 1000)
}
